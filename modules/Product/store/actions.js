  ...
  $nuxt.$nextTick(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (
          res.data.grid[0] &&
          $nuxt.$store.state.Layouts.config.product.selectSizeIfOnlyOne
        ) {
...
const makeDaysProduct = function() {
  let days = 0;
  if (days === 0 || days === null) {
    return false;
  } else {
...
const calculateProductDelivery = function({ commit, state }, cep) {
  ...
  let data = {
    grids: [
      {
        grid_id: state.product.grid[0].id,
        amount: 1,
      },
    ],
    cep: cep,
  ...
  axios
    .post(`${baseApiUrl}/v2/ecommerce/delivery/check`, data)
...
const checkIfHaveRequiredAtributes = function() {
  ...
  if (
    $nuxt.$store.state.Layouts.config.atribute &&
    $nuxt.$store.state.Layouts.config.atribute.show
  ) {
    let atributeRequiredFields = $nuxt.$store.state.Layouts.config.atribute.atributes.filter(
      (value) => value.required == true
    );
    atributeRequiredFields.map(function(elem) {
      var requiredField = $nuxt.$store.state.Product.product.product_atribute.filter(
        (value) => value.group_name == elem.atributeName
      )[0];
      if (requiredField !== undefined) {
        if (
          $nuxt.$store.state.Product.atributes.filter(
            (value) => value.group_name == requiredField.group_name
          ).length == 0 &&
          !requiredEmpty
        ) {
          alert(
            `O campo ${requiredField.group_name} é requerido para este produto !!`
          );
          requiredEmpty = true;
          // return false;
        }
...
const sendWarnMeData = function({ commit }, payload) {
  ...
  axios
    .post(`${baseApiUrl}/ecommerce/product/warn-me`, data)
    .then((res) => {
      if (res.data.status == 201) {
        commit(
          "SET_WARNME_TOAST_MESSAGE",
          "Certo, você será avisado(a) assim que o produto estiver disponível!"
        );
      } else {
