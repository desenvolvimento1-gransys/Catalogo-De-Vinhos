...
const getSearchParams = function ({ commit }) {
  ...
  if (!data || data.length === 0) {
    console.error("Nenhum dado de rotas encontrado.");
    return {}; // Retorna um objeto vazio como fallback
  }

  if ($nuxt.$route.path == '/') {
    return $nuxt.$store.state.Settings.home.searchParams || {}; // Retorne um valor padrão se `searchParams` estiver indefinido
  } else {
    ...
    if (result.length === 0) {
      console.error(`Nenhuma rota encontrada para o caminho: ${path}`);
      return {}; // Retorna um objeto vazio como fallback
    }
    ...
    if (result[0] && result[0].params) {
      ...
      console.error(`Nenhum parâmetro encontrado para a rota: ${path}`);
